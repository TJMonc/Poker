...
int main() {
	...
	std::cout << std::format("No{}", players[0].getHandType());
				...
				for (size_t i = 0; i < 4; i++) {
					...
					if (winner->getHandType() < players[i].getHandType()) {
						...
						if (winner->getHandType() == Poker::Hand::Flush) {
							if (players[i].getHighSuit() > winner->getHighSuit()) {
								winner = &players[i];
								winnerIndex = i;
							}
						}
		...
		if (players[turn].getIsPlayer()) {
			if ((turn == 0 || turn == 2)) {
				...
				if (Mouse::isButtonPressed(Mouse::Left) && lol > 30) {
					...
					else if (mouseCircle.getGlobalBounds().intersects(foldBox.getGlobalBounds()) && !isRaising[turn]) {
						foldPressed = true;
					}
